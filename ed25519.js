...
function getExtendedPublicKey(key) {
  ...
  key =
    typeof key === 'bigint' || typeof key === 'number'
      ...
      : ensureBytes(key)
  if (key.length !== 64) throw new Error('Expected 64 bytes')
  // hash to produce 64 bytes
  // First 32 bytes of 64b uniformingly random input are taken,
  // clears 3 bits of it to produce a random field element.
  const head = adjustBytes25519(key.slice(0, 32))
  // Second 32 bytes is called key prefix (5.1.6)
  const prefix = key.slice(32, 64)
 ...
 * RFC8032 5.1.5
 */
window.ed25519.getPublicKey = function getPublicKey(privateKey) {
 ...
 * RFC8032 5.1.6
 */
window.ed25519.sign = function sign(message, privateKey) {
 ...
 */
 window.ed25519.verify = function verify(sig, message, publicKey) {
  message = ensureBytes(message)
 ...
 */
 window.ed25519.getSharedSecret = function getSharedSecret(privateKey, publicKey) {
  const { head } = getExtendedPublicKey(privateKey)
  const u = Point.fromHex(publicKey).toX25519()
  return curve25519.scalarMult(head, u);
}

window.ed25519.toX25519 = function toX25519(publicKey) {
...
const utils = {
  ...
  hash: (message) => {
    ...
    return window.blake2b.blake2b(message);
  },
  /**
